#!/usr/bin/env python
"""Download and verify base-model weights for the PubMedCLIP biomedical pipeline.

Downloads and verifies the pinned Hugging Face PubMedCLIP checkpoint
...
Usage:
    ...
    # Download the default model into weights/pubmed-clip-vit-base-patch32/:
    python scripts/fetch_weights.py

    # Download to an explicit custom destination directory:
    python scripts/fetch_weights.py --dest weights/pubmed-clip-vit-base-patch32
...
from pubmedclip_pipeline.config import (  # noqa: E402
    DEFAULT_MODEL_KEY,
    MODEL_FILENAME,
    MODEL_ID,
    MODEL_REVISION,
...
def matches_patterns(
    ...
    """Check if filename matches allow patterns and does not match ignore patterns. The one pinned
    PyTorch source (`SOURCE_FILENAME`, audited and converted once by the package) is the only pickle
    allowed; every other `.bin` is ignored."""
...
def generate_manifest(
    ...
    for p in sorted(model_dir.iterdir()):
        if p.is_file() and p.name not in (MANIFEST_NAME, MODEL_FILENAME):
            # MODEL_FILENAME is the converted serving file, not a Hub file
            if p.is_symlink():
...
def verify_snapshot(
    ...
    for record in files:
        ...
        if path_str not in actual_files:
            if path_str == SOURCE_FILENAME and MODEL_FILENAME in actual_files:
                continue  # converted-only (DIMER-hosted) shape: the source pickle is not required
    ...
    # The converted serving file is verified against the package constants whenever it is present.
    converted = actual_files.get(MODEL_FILENAME)
    if converted is not None and MODEL_FILENAME not in {r.get("path") for r in files}:
        actual_size = converted.stat().st_size
        if actual_size != expected_weight_bytes:
            errors.append(
                f"Actual {MODEL_FILENAME} size ({actual_size}) != config {expected_weight_bytes}"
            )
        else:
            actual_sha256 = sha256_file(converted)
            if actual_sha256 != expected_weight_sha256:
                errors.append(
                    f"Actual {MODEL_FILENAME} sha256 ({actual_sha256}) != "
                    f"config {expected_weight_sha256}"
    ...
    for name, _p in actual_files.items():
        if name not in (MANIFEST_NAME, MODEL_FILENAME) and name not in manifested_names:
...
def fetch_model(
    ...
    """Download and verify the PubMedCLIP model checkpoint."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download and verify base-model weights for the PubMedCLIP pipeline."
    )
    parser.add_argument(
        "--dest",
        type=Path,
