...
6. Execution policy: `trust_remote_code=False`, `use_safetensors=True`, `local_files_only=True`; the source pickle is tolerated in the snapshot directory only as the digest-verified conversion input and is never passed to transformers; with the converted file present it is not required (the shape of a deployment that ships only the converted file)
