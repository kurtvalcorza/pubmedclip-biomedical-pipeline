...
def test_release_notebook_declares_e2e_profile() -> None:
    ...
    assert "pubmedclip_biomedical_colab.ipynb" in registry
...
def test_release_notebook_loads_on_the_available_device() -> None:
    ...
    load_expr = "pipe = PubMedClipPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, progress=print)"
    assert load_expr in source
