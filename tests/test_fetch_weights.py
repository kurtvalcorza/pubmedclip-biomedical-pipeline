...
def test_matches_patterns():
    ...
    assert matches_patterns("pytorch_model.bin")  # the one pinned source, converted once
    ...
    assert not matches_patterns("other_model.bin")
    assert not matches_patterns("tf_model.h5")
    assert not matches_patterns("flax_model.msgpack")
...
def test_generate_manifest_and_verify_snapshot(tmp_path: Path):
    ...
    file_b = tmp_path / "pytorch_model.bin"  # the pinned source is a manifest (Hub) entry
    file_b.write_bytes(b"dummy source content")
    converted = tmp_path / "model.safetensors"  # the converted file is not a Hub entry
    ...
    assert [f["path"] for f in manifest["files"]] == ["config.json", "pytorch_model.bin"]
...
def test_committed_base_model_snapshot_manifest_matches_weights():
    ...
    has_source = (DEFAULT_DEST_DIR / "pytorch_model.bin").is_file()
    if not has_source and not (DEFAULT_DEST_DIR / MODEL_FILENAME).is_file():
        # In CI/fresh clone without weights downloaded,
        # only the pinned source and total bytes should mismatch
        assert ok is False
        assert any("Missing file: pytorch_model.bin" in err for err in errors)
        non_weight_errors = [
            err
            for err in errors
            if "pytorch_model.bin" not in err and "Total bytes mismatch" not in err
        ]
...
def test_verify_snapshot_enforces_pinned_model_safetensors_config_constants(tmp_path: Path):
    from pubmedclip_pipeline.config import MODEL_SHA256, MODEL_SIZE_BYTES
